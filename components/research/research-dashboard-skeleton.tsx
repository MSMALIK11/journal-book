...
export function ResearchDashboardSkeleton() {
  return (
    <div className="space-y-6">
      ...
      <Skeleton className="h-10 w-full max-w-md bg-cyan-400/10" />
      <Skeleton className="h-28 w-full rounded-xl bg-cyan-400/10" />
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <Skeleton key={index} className="h-24 rounded-xl bg-cyan-400/10" />
        ))}
      </div>
