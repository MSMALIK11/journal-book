...
JBSync.scrapeFromActiveTab = async function scrapeFromActiveTab(importAll = false) {
  ...
  try {
    ...
    const trades = JBSync.applyChartSymbol(
      [...merged.values()].sort((a, b) => b.tradeNumber - a.tradeNumber),
      chartSymbol,
    )
...
JBSync.syncTrades = async function syncTrades(trades, config, chartSymbol) {
  ...
  return { imported, updated, skipped, deduped, byAccount, accountsCreated: [...accountsCreated] }
}
